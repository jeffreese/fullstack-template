...
generated by better-auth (user, session, account, verification) plus an example
`posts` table to show the pattern:
...
1. Define the table in `app/db/schema.ts`
...
The seed script at `app/db/seed.ts` populates the database with sample data for
